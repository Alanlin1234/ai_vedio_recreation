...
- 只输出一个 JSON 对象，不要 Markdown：{{"scene_count": <整数>, "rationale": "<不超过80字的中文说明>"}}"""
    ...
    def generate_storyboard(
        ...
        try:
            ...
            scenes = self._generate_storyboard_scripts(
                story_content, scene_count, debug_prompts, language=language
            )
            ...
            # 第二步：生成视觉风格指南
            style_guide = self._generate_style_guide(story_content, debug_prompts, language=language)
            ...
            if self._non_human_primary_cast(story_content, scenes, style_guide):
                ...
                if language == 'en':
                    style_guide['character_description'] = (
                        f"Animal or anthropomorphic animal cast only; no humans or photoreal people. "
                        f"Lead look must match script: {v0 or 'see each shot visual_subject'}."
                    )
                else:
                    style_guide['character_description'] = (
    ...
    def _generate_storyboard_scripts(
        ...
        try:
            ...
            if language == 'en':
                prompt = f"""From the story below, output **exactly {scene_count}** storyboard shots. Each shot is ~{SHOT_DURATION_SECONDS}s of screen time; describe what happens in that window (plot, action, camera, spoken lines).

Story:
{story_content[:8000]}

Hard rules:
1. Exactly {scene_count} shots in **chronological order**, one continuous short film (setup–development–payoff). No skipped causality; scene changes need a clear bridge.
2. **Cast consistency**: decide if cast is humans / animals / anthropomorphic animals. `visual_subject`, `description`, `plot` must use the **same** characters; no species drift between fields or shots.
3. **visual_subject**: one line per shot — who/what must appear on screen (species, age feel, outfit or fur colors), aligned with the script.
4. **plot**: at least 3 sentences, preferably 4–6, in **English**, covering this {SHOT_DURATION_SECONDS}s beat: opening beat → conflict/action → end-of-shot result; shot 2+ must continue from the previous shot.
5. **action_in_shot**: visible physical action in this {SHOT_DURATION_SECONDS}s window; must match what `description` can draw.
6. **camera_movement**: how the camera moves (lock-off/push/pull/pan/track/etc.) and how it connects to the prior shot; align with `shot_type`.
7. **description**: composition, subject placement, environment, light; drawable; matches `visual_subject`.
8. **dialogue**: **required** every shot — spoken line, inner thought, or "Narrator: ...". For animal fables, anthropomorphic dialogue is OK. Never empty, "none", or "no dialogue"; if purely action, use "Character: (internal) ..." or "(Ambience: wind)".
9. **shot_type**: concrete framing (e.g. extreme wide high angle, medium side profile, close-up); avoid repeating identical framing+composition back-to-back.
10. **prompt** (image-to-video): **English only**, include motion and camera; you may prefix "Story beat:" / "Camera:".
11. Each shot `duration` must be {SHOT_DURATION_SECONDS}.

Return **only** a JSON array (no markdown), keys exactly:
[{{"scene_number":1,"shot_type":"...","visual_subject":"...","description":"...","action_in_shot":"...","camera_movement":"...","plot":"...","dialogue":"...","prompt":"...","duration":{SHOT_DURATION_SECONDS}}}]"""

                sys_story = (
                    "You are a film storyboard artist. Consistent cast; continuous story; ~5s per shot with action, "
                    "camera, and mandatory English dialogue; I2V prompts in English with clear continuity."
                )
                trace_sys = "Storyboard artist: consistent species; shot continuity; English dialogue; English I2V prompts."
            else:
...
[{{"scene_number":1,"shot_type":"景别","visual_subject":"本镜画面必须出现的主体","description":"画面","action_in_shot":"本5秒内动作","camera_movement":"本5秒内镜头运动","plot":"情节节拍","dialogue":"台词或旁白","prompt":"图生视频提示词","duration":{SHOT_DURATION_SECONDS}}}]"""
                ...
                sys_story = (
                    "你是电影分镜师。分镜须物种与画面一致、连成完整故事、镜间有因果承接；每镜写满约5秒内的动作与镜头；台词必填；视频提示词须体现叙事衔接。"
                )
                trace_sys = '你是电影分镜师。物种与画面一致；镜间承接；每镜约5秒含动作与镜头；台词必填。'
    ...
    def _generate_style_guide(
        ...
        try:
            ...
            if language == 'en':
                prompt = f"""From the story, produce a unified visual style guide (English) for consistent storyboard images.

Story:
{story_content}

**character_description** must match species in the text — if fable/animals, say anthropomorphic rabbit/fox etc.; **do not** default to young human woman if the script has no humans.

Return JSON only:
{{
    "character_description": "cross-shot look: species, age feel, outfit/fur, iconic props (match source; no inventing humans for animal leads)",
    "color_scheme": "palette",
    "lighting_style": "key lighting approach",
    "scene_setting": "environment style",
    "art_style": "e.g. cinematic, picture-book, 3D animation",
    "visual_mood": "overall mood"
}}"""

                sg_sys = "Visual style designer. Species must match the script; never replace animal cast with default humans."
                sg_trace = "Professional visual style designer (English JSON)."
            else:
...
{{
    ...
    "visual_mood": "视觉氛围 - 描述整体的情绪和氛围"
                ...
                sg_sys = "你是视觉风格设计师。物种与剧本一致，禁止用默认真人替代故事中的动物或拟人角色。"
                sg_trace = '你是一个专业的视觉风格设计师。'
    ...
    def _generate_ref_storyboard_image_task(
        ...
        if language == 'en':
            ref_instructions = """[qwen-image-edit brief]
The reference image is **shot 1**. Edit-generate a new frame: **keep** the same lead species, fur/outfit, and overall style; **follow the text below** to change framing, camera, action, background, and story beat.

[Rules]
1. Match this shot's beat, description, framing, and subject line; show unique action and composition.
2. Must differ clearly from the reference (new lens / action), not a crop or tiny tweak.
3. Species and identity stay consistent with the reference unless the text explicitly introduces a new character.
4. No text, captions, signs, watermarks, or logos in the image.
