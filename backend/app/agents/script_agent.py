...
class ScriptAgent(BaseAgent):
    ...
    async def _generate_with_llm(self, hotspot: Dict, style: str, duration: int) -> str:
        ...
        # API配置(使用gpt-4o模型)
        api_key = self.config.get('llm_api_key', 'sk-93b182196961412daaddce47c7f33507')
        api_endpoint = self.config.get('llm_api_endpoint', 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions')
        model = self.config.get('llm_model', 'qwen3-max')
